...
\item In a \textbf{network with multiple edges} ($=$), 
  ...
  type of network is also called a multi  
...
Table~\ref{tab:matcross} gives a more concise overview. 
...
\begin{table}
  ...
  \begin{tabular}{ l l c@{\;}l c@{\;}l c }
    ...
    & Symmetric adjacency matrix			& $\mathbf A$         && $\mathbf N$       && $\mathbf P$ \\
    & Symmetric Laplacian		  		& $\mathbf L$         && $\mathbf Z$       && $\mathbf S$ \\
    & Symmetric signless Laplacian			& $\mathbf K$         && $\mathbf C$       && $\mathbf J$ \\
    \cmidrule(lr){2-7}
    ...
    & Unidirectional signless Laplacian$\dagger$		& $\vec{\mathbf K}$   && \\
    \midrule
...
A tabular overview is given in Table~\ref{tab:matov}. 
...
\begin{table}
  ...
  \begin{tabular}{ l l c@{\;}l l l l }
    \toprule
    & \textbf{Int.\ name} & \multicolumn{2}{l}{\textbf{Expression}} &
    \textbf{Name} & \textbf{Properties} & \textbf{Eigenvalues} \\
    \midrule

    \ref{sec:matrix.A} & \texttt{sym} & $\mathbf A$ && Adjacency m. &
    Symmetric             			& $\alpha$ \\
    \ref{sec:matrix.L} & \texttt{lap} & $\mathbf L$&$ = \mathbf D - \mathbf A$ & Laplacian m. &
    Symmetric, positive-semidefinite  		& $\xi, a$ \\ 
    \ref{sec:matrix.K} & \texttt{lapq} & $\mathbf K$&$ = \mathbf D + \mathbf A$ & Signless Laplacian m. &
    Symmetric, positive-semidefinite            & $\chi$ \\

    \midrule

    \ref{sec:matrix.N} & \texttt{sym-n} & $\mathbf N$&$ = \mathbf D^{-\frac 12}\mathbf A\mathbf D^{-\frac 12}$ & Norm.\ adjacency m. &
    Symmetric, eigenvalues in $[-1,+1]$		& $1$  \\
    \ref{sec:matrix.Z} & * & $\mathbf Z$&$ = \mathbf I - \mathbf N$ & Norm.\ Laplacian m. &
    Symmetric, eigenvalues in $[0,2]$		& $?,?$ \\
    \ref{sec:matrix.C} & * & $\mathbf C$&$ = \mathbf I + \mathbf N$ & Norm.\ signless Laplacian m. &
    Symmetric, eigenvalues in $[0,2]$		& $?$ \\

    \midrule

    \ref{sec:matrix.P} & \texttt{stoch1} & $\mathbf P$&$=\mathbf D^{-1}\mathbf A$ & Stoch.\ adjacency m. &
    Right-stochastic, eigenvalues in $[-1,+1]$	& $\dagger$ \\
    \ref{sec:matrix.S} & * & $\mathbf S$&$ =\mathbf I - \mathbf P$ & Stoch.\ Laplacian m. &
    Eigenvalues in $[0,2]$			& $\dagger$ \\
    \ref{sec:matrix.J} & * & $\mathbf J$&$ = \mathbf I + \mathbf P$ & Stoch.\ signless Laplacian m. &
    Eigenvalues in $[0,2]$ 			& $\dagger$ \\
  ...
  * These decompositions can be derived from \texttt{sym-n} and
  \texttt{stoch1} by a transformation of eigenvalues.  \\
  $\dagger$ The eigenvalues of the stochastic matrices are identical to
  the eigenvalues of their normalized analogues.  \\
  (A) The largest absolute eigenvalues are considered. \\
  (0) The smallest absolute eigenvalues are considered. 
\end{table}
